    ...
    <div className="space-y-8">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="minimumPayment">{t('calculator.minimumPayment')}</Label>
            <Input
              id="minimumPayment"
              type="number"
              min="0"
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">{t('calculator.balance')}</TableHead>
                  <TableHead className="text-right">{t('calculator.interestRate')}</TableHead>
                  <TableHead className="text-right">{t('calculator.minimumPayment')}</TableHead>
                  <TableHead className="text-center">{t('calculator.actions')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {debts.map((debt) => (
                ...
                <TableRow className="bg-muted/50">
                  ...
                  <TableCell className="text-right font-medium">{formatCurrency(totalMinimumPayment)}</TableCell>
                  <TableCell></TableCell>
                </TableRow>
              </TableBody>
            </Table>
      ...
      {debts.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-xl font-bold">{t('calculator.paymentStrategy')}</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Tabs value={strategy} onValueChange={handleStrategyChange}>
                <TabsList className="grid grid-cols-2">
                  <TabsTrigger value="avalanche">{t('calculator.avalanche')}</TabsTrigger>
                  <TabsTrigger value="snowball">{t('calculator.snowball')}</TabsTrigger>
                </TabsList>
                
                <TabsContent value="avalanche" className="space-y-2 mt-4">
                  <h4 className="font-semibold">{t('calculator.avalancheStrategy')}</h4>
                ...
                <TabsContent value="snowball" className="space-y-2 mt-4">
                  <h4 className="font-semibold">{t('calculator.snowballStrategy')}</h4>
                  <p className="text-muted-foreground">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-muted/50 p-4 rounded-lg">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-semibold">{formatCurrency(totalMinimumPayment)}</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span>{t('calculator.additionalPayment')}:</span>
                  <span className="font-semibold">{formatCurrency(additionalPayment)}</span>
                </div>
      ...
      {paymentPlan && (
        <div className="space-y-4">
          <h3 className="text-xl font-bold">{t('calculator.paymentResults')}</h3>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <CardHeader>
                <CardTitle>{t('calculator.payoffDate')}</CardTitle>
                <CardDescription>{t('calculator.estimatedCompletion')}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-3xl font-bold">{formatDate(paymentPlan.payoffDate)}</div>
                <p className="text-muted-foreground">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t('calculator.interestPaid')}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-3xl font-bold">{formatCurrency(paymentPlan.totalInterestPaid)}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t('calculator.principalPlusInterest')}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="text-3xl font-bold">{formatCurrency(paymentPlan.totalPaid)}</div>
              </CardContent>
            </Card>
          </div>
