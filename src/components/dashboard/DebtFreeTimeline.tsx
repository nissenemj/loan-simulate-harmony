...
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ComposedChart, Line, ReferenceLine } from 'recharts';
...
const DebtFreeTimeline = ({
    ...
    const loanMinPayments = activeLoans.reduce((sum, loan) => {
      if (loan.minPayment && loan.minPayment > 0) {
        ...
        if (loan.repaymentType === 'custom-payment' && loan.customPayment) {
          ...
          payment = loan.amount * monthlyRate * Math.pow(1 + monthlyRate, totalMonths) / (Math.pow(1 + monthlyRate, totalMonths) - 1);
        }
        if (loan.monthlyFee) {
  ...
  const [paymentAmount, setPaymentAmount] = useState(Math.max(monthlyBudget || 0, safeMinPayment));
  ...
  const strategies: StrategyResult[] = useMemo(() => [{
    id: 'avalanche',
    name: t('repayment.avalancheStrategy'),
    months: avalanchePlan.totalMonths,
    interest: avalanchePlan.totalInterestPaid,
    date: avalancheDate
  }, {
    id: 'snowball',
    name: t('repayment.snowballStrategy'),
    months: snowballPlan.totalMonths,
    interest: snowballPlan.totalInterestPaid,
    date: snowballDate
  }, {
    id: 'equal',
    name: t('dashboard.equalDistribution'),
    months: equalPlan.totalMonths,
    interest: equalPlan.totalInterestPaid,
    date: equalDate
  ...
  const transformTimelineData = (timeline: any[], strategyId: string): ChartDataPoint[] => {
    ...
    if (totalPeriods <= 36) {
      return timeline.map((point, index) => {
        const monthlyInterest = index > 0 ? point.totalInterestPaid - timeline[index - 1].totalInterestPaid : point.totalInterestPaid;
        const monthlyPrincipal = index > 0 ? timeline[index - 1].totalRemaining - point.totalRemaining : 0;
    ...
    for (let i = 0; i < totalPeriods; i++) {
      ...
      if (isRegularSample || isLastPoint || isSignificantChange) {
        if (i - lastIncludedMonth >= samplingRate / 2 || isLastPoint) {
          lastIncludedMonth = i;
          const monthlyInterest = i > 0 ? timeline[i].totalInterestPaid - timeline[i - 1].totalInterestPaid : timeline[i].totalInterestPaid;
          const monthlyPrincipal = i > 0 ? timeline[i - 1].totalRemaining - timeline[i].totalRemaining : 0;
          const monthlyPayment = monthlyInterest + monthlyPrincipal;
  ...
  const avalancheTimelineData = useMemo(() => transformTimelineData(getSelectedPlan('avalanche').timeline, 'avalanche'), [avalanchePlan]);
  const snowballTimelineData = useMemo(() => transformTimelineData(getSelectedPlan('snowball').timeline, 'snowball'), [snowballPlan]);
  const equalTimelineData = useMemo(() => transformTimelineData(getSelectedPlan('equal').timeline, 'equal'), [equalPlan]);
  ...
  const combinedTimelineData = useMemo(() => {
    ...
    const primaryData = selectedStrategy === 'avalanche' ? avalancheTimelineData : selectedStrategy === 'snowball' ? snowballTimelineData : equalTimelineData;
    const comparisonData = comparisonStrategy === 'avalanche' ? avalancheTimelineData : comparisonStrategy === 'snowball' ? snowballTimelineData : equalTimelineData;
    ...
    return combined;
  }, [selectedStrategy, comparisonStrategy, showComparison, avalancheTimelineData, snowballTimelineData, equalTimelineData]);
  ...
  const maxValues = useMemo(() => {
    ...
    if (showComparison && comparisonStrategy) {
      maxComparisonBalance = Math.max(...chartData.filter(point => point[`balance_${comparisonStrategy}`] !== undefined).map(point => point[`balance_${comparisonStrategy}`] as number));
      maxComparisonInterest = Math.max(...chartData.filter(point => point[`totalInterest_${comparisonStrategy}`] !== undefined).map(point => point[`totalInterest_${comparisonStrategy}`] as number));
    }
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      ...
      return <div className="bg-background border rounded-md p-3 shadow-md">
          ...
          <div className="mt-2 border-b pb-2">
            ...
            {payload.find(p => p.dataKey === 'balance') && <p className="text-primary flex justify-between">
                <span>{t('dashboard.remainingDebt')}:</span> 
                <span className="font-mono">{formatCurrency(payload.find(p => p.dataKey === 'balance').value)}</span>
              </p>}
            {payload.find(p => p.dataKey === 'principal') && <p className="text-green-600 flex justify-between">
                <span>{t('dashboard.principal')}:</span>
                <span className="font-mono">{formatCurrency(payload.find(p => p.dataKey === 'principal').value)}</span>
              </p>}
            {payload.find(p => p.dataKey === 'monthlyInterest') && <p className="text-destructive flex justify-between">
            ...
            {payload.find(p => p.dataKey === 'totalInterest') && <p className="text-amber-600 flex justify-between">
            ...
            {payload.find(p => p.dataKey === 'monthlyPayment') && <p className="text-blue-600 flex justify-between">
          ...
          {/* Comparison strategy */}
          {showComparison && comparisonStrategy && <div className="mt-2">
              <p className="font-medium">{t(`repayment.${comparisonStrategy}Strategy`)}</p>
              {payload.find(p => p.dataKey === `balance_${comparisonStrategy}`) && <p className={`flex justify-between text-${getStrategyColor(comparisonStrategy, 'balance')}`}>
                  <span>{t('dashboard.remainingDebt')}:</span>
                  <span className="font-mono">{formatCurrency(payload.find(p => p.dataKey === `balance_${comparisonStrategy}`).value)}</span>
                </p>}
              {payload.find(p => p.dataKey === `totalInterest_${comparisonStrategy}`) && <p className={`flex justify-between text-${getStrategyColor(comparisonStrategy, 'interest')}`}>
                  <span>{t('dashboard.totalInterestPaid')}:</span>
  ...
  const exportToCSV = () => {
    ...
    chartData.forEach(point => {
      ...
      let row = [point.month, date, point.balance, point.principal, point.monthlyInterest, point.totalInterest, point.monthlyPayment];
      if (showComparison && comparisonStrategy) {
        row.push(point[`balance_${comparisonStrategy}`] || 0, point[`totalInterest_${comparisonStrategy}`] || 0);
      }
      csvContent += row.join(",") + "\r\n";
    });
  ...
  const handleStrategyVisibilityChange = (strategy: 'avalanche' | 'snowball' | 'equal') => {
    ...
    const wouldAllBeHidden = Object.entries(visibleStrategies).filter(([key, value]) => key !== strategy ? value : !value).every(([_, value]) => !value);
    if (wouldAllBeHidden) {
  ...
  return <Card className="overflow-hidden" ref={chartRef}>
      <CardHeader>
      ...
      <CardContent>
        <div className="space-y-6">
          {!hasDebt ? <div className="p-4 text-center">
              ...
              <div className="space-y-3">
                <label className="text-sm font-medium flex items-center justify-between">
                  ...
                  {!isValidPayment && <span className="text-destructive flex items-center text-xs">
                      <AlertCircle className="h-3 w-3 mr-1" />
                      {t('repayment.minPaymentRequired')}
                    </span>}
                </label>
                <Slider value={[paymentAmount]} min={safeMinPayment} max={Math.max(safeMinPayment * 3, monthlyBudget * 2, safeMinPayment + 500)} step={10} onValueChange={value => setPaymentAmount(value[0])} className={`w-full ${!isValidPayment ? 'border-destructive' : ''}`} />
              ...
              {recommendedStrategy && isValidPayment && recommendedStrategy.months > 0 && <div className="bg-primary/5 p-4 rounded-md">
              ...
              {showComparison && <div className="border rounded-md p-3 bg-muted/20">
                  <p className="font-medium text-sm mb-2">{t('dashboard.selectComparisonStrategy')}</p>
                  <div className="flex flex-wrap gap-2">
                    {strategies.map(strategy => strategy.id !== selectedStrategy && <Button key={strategy.id} variant={comparisonStrategy === strategy.id ? "default" : "outline"} size="sm" onClick={() => setComparisonStrategy(strategy.id)} className="text-xs">
                          {strategy.name}
                        </Button>)}
                  </div>
                </div>}
              
              {isValidPayment && <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                  {strategies.map(strategy => <Card key={strategy.id} className={`cursor-pointer transition-all hover:shadow ${getStrategyCardClass(strategy.id)}`} onClick={() => setSelectedStrategy(strategy.id)}>
                      <CardHeader className="p-3">
                        <CardTitle className="text-base flex items-center justify-between">
                          {strategy.name}
                          <Checkbox checked={visibleStrategies[strategy.id]} onCheckedChange={() => handleStrategyVisibilityChange(strategy.id)} onClick={e => e.stopPropagation()} aria-label={`Show ${strategy.name} on chart`} />
                        </CardTitle>
                      </CardHeader>
              ...
              {chartData.length > 0 && isValidPayment && <div className="mt-4">
                  <div className="h-80">
                ...
                comparisonInterest: {
                  ...
                  bottom: 10
                    ...
                    fontSize: 12
                        ...
                        <YAxis yAxisId="left" tickFormatter={value => `${Math.round(value / 1000)}k`} label={{
                    value: t('dashboard.debt'),
                    angle: -90,
                    position: 'insideLeft'
                  }} domain={[0, maxValues.balance]} tick={{
                    fontSize: 12
                  }} />
                        <YAxis yAxisId="right" orientation="right" tickFormatter={value => `${Math.round(value / 1000)}k`} label={{
                    value: t('dashboard.interest'),
                    angle: 90,
                    position: 'insideRight'
                  }} domain={[0, maxValues.interest]} tick={{
                    fontSize: 12
                  }} />
                        
                        <ReferenceLine x={currentMonth} stroke="#888" strokeWidth={2} label={{
                    ...
                    fill: '#888'
                        ...
                        {visibleStrategies[selectedStrategy] && <Area yAxisId="left" type="monotone" dataKey="balance" name={`${t('dashboard.debt')} (${t(`repayment.${selectedStrategy}Strategy`)})`} stroke={getStrategyColor(selectedStrategy, 'balance')} fillOpacity={0.3} fill={getStrategyColor(selectedStrategy, 'balance')} strokeWidth={2} />}
                        
                        {/* Principal Payments */}
                        {visibleStrategies[selectedStrategy] && <Area yAxisId="right" type="monotone" dataKey="principal" name={t('dashboard.principal')} stroke={getStrategyColor(selectedStrategy, 'principal')} fillOpacity={0.5} fill={getStrategyColor(selectedStrategy, 'principal')} stackId="payments" />}
                        
                        {/* Interest Payments */}
                        {visibleStrategies[selectedStrategy] && <Area yAxisId="right" type="monotone" dataKey="monthlyInterest" name={t('dashboard.monthlyInterest')} stroke={getStrategyColor(selectedStrategy, 'interest')} fillOpacity={0.5} fill={getStrategyColor(selectedStrategy, 'interest')} stackId="payments" />}
                        
                        {/* Total Interest Line */}
                        {visibleStrategies[selectedStrategy] && <Line yAxisId="right" type="monotone" dataKey="totalInterest" name={`${t('dashboard.totalInterestPaid')} (${t(`repayment.${selectedStrategy}Strategy`)})`} stroke={getStrategyColor(selectedStrategy, 'interest')} dot={false} activeDot={{
                    r: 6
                  }} strokeWidth={2} />}
                        
                        {/* Comparison Strategy - if enabled */}
                        {showComparison && comparisonStrategy && visibleStrategies[comparisonStrategy] && <>
                            <Line yAxisId="left" type="monotone" dataKey={`balance_${comparisonStrategy}`} name={`${t('dashboard.debt')} (${t(`repayment.${comparisonStrategy}Strategy`)})`} stroke={getStrategyColor(comparisonStrategy, 'balance')} strokeDasharray="5 5" dot={false} activeDot={{
                      r: 6
                    }} strokeWidth={2} />
                            <Line yAxisId="right" type="monotone" dataKey={`totalInterest_${comparisonStrategy}`} name={`${t('dashboard.totalInterestPaid')} (${t(`repayment.${comparisonStrategy}Strategy`)})`} stroke={getStrategyColor(comparisonStrategy, 'interest')} strokeDasharray="5 5" dot={false} activeDot={{
                      r: 6
                    }} strokeWidth={2} />
                          </>}
                      </ComposedChart>
              ...
              {isValidPayment && getSelectedPlan(selectedStrategy).totalMonths > 0 && <div className="relative">
                  ...
                  {activeCards.length > 0 && getSelectedPlan(selectedStrategy).creditCardFreeMonth && <div className="relative pl-8 pb-6">
      ...
      <CardFooter className="flex flex-col gap-2 items-stretch sm:flex-row sm:items-center">
        ...
        <Button variant="default" onClick={() => navigate('/debt-summary?tab=repayment-plan')} className="flex items-center">
