...
const LandingPageDemo = () => {
  ...
  const calculateResults = async () => {
    try {
      ...
      const avalanchePlan = await calculatePaymentPlan(debts, totalPayment, 'avalanche');
      
      // Calculate for snowball strategy
      const snowballPlan = await calculatePaymentPlan(debts, totalPayment, 'snowball');
      ...
      // Prepare chart data
      const currentStrategy = strategy === 'avalanche' ? avalanchePlan : snowballPlan;
  ...
  const currentResults = strategy === 'avalanche' ? results.avalanche : results.snowball;
  const alternativeResults = strategy === 'avalanche' ? results.snowball : results.avalanche;
  ...
  const calculateSavings = () => {
    ...
    try {
      ...
      const minimumPaymentSum = debts.reduce((sum, debt) => sum + debt.minimumPayment, 0);
      const minPaymentPlan = calculatePaymentPlan(debts, minimumPaymentSum, 'avalanche');
      
      return {
        monthsSaved: minPaymentPlan.totalMonths - currentResults.months,
        interestSaved: minPaymentPlan.totalInterestPaid - currentResults.interest
      };
    } catch (error) {
  ...
  const strategyComparison = () => {
    if (!currentResults || !alternativeResults) return { monthsDiff: 0, interestDiff: 0 };
    
    return {
      monthsDiff: Math.abs(currentResults.months - alternativeResults.months),
      interestDiff: Math.abs(currentResults.interest - alternativeResults.interest)
    };
  };
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>{t('landing.demo.calculatorDescription')}</CardDescription>
          </CardHeader>
          
          <CardContent className="space-y-6">
            {/* Debt inputs */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">{t('landing.demo.debtInputsTitle')}</h3>
              
              {debts.map((debt, index) => (
                <div key={debt.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border rounded-md">
                  <div>
                    <Label htmlFor={`demo-debt-name-${index}`}>{t('loan.name')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor={`demo-debt-balance-${index}`}>{t('loan.balance')}</Label>
                  ...
                  <div>
                    <Label htmlFor={`demo-debt-interest-${index}`}>{t('loan.interestRate')}</Label>
                    <div className="relative">
                      <Input
                  ...
                  <div>
                    <Label htmlFor={`demo-debt-payment-${index}`}>{t('repayment.minPayment')}</Label>
                    <div className="relative">
                      <span className="absolute inset-y-0 left-3 flex items-center text-muted-foreground">€</span>
                      <Input
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-lg font-medium mb-2">{t('landing.demo.extraPaymentTitle')}</h3>
                <p className="text-sm text-muted-foreground mb-4">{t('landing.demo.extraPaymentDescription')}</p>
            ...
            {/* Strategy selection */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium mb-2">{t('landing.demo.strategyTitle')}</h3>
              <p className="text-sm text-muted-foreground mb-4">{t('landing.demo.strategyDescription')}</p>
              
              <Tabs value={strategy} onValueChange={(value) => setStrategy(value as PaymentStrategy)}>
                <TabsList className="grid grid-cols-2 w-full max-w-md mx-auto">
                  <TabsTrigger value="avalanche">{t('repayment.avalancheMethod')}</TabsTrigger>
                  <TabsTrigger value="snowball">{t('repayment.snowballMethod')}</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="avalanche" className="mt-4">
                  <div className="bg-muted/50 p-4 rounded-md">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm">{t('landing.demo.avalancheDescription')}</p>
                    </div>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="snowball" className="mt-4">
                  <div className="bg-muted/50 p-4 rounded-md">
                    <div className="flex items-start gap-2">
                      <Lightbulb className="h-5 w-5 text-primary mt-0.5" />
                      <p className="text-sm">{t('landing.demo.snowballDescription')}</p>
                    </div>
                  </div>
                </TabsContent>
              </Tabs>
            ...
            {currentResults && (
              <div className="space-y-6 pt-4">
                <div className="border-t pt-6">
                  <h3 className="text-lg font-medium mb-4">{t('landing.demo.resultsTitle')}</h3>
                    ...
                    <Card className="bg-muted/30">
                      <CardContent className="pt-6">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <h4 className="font-medium">{t('landing.demo.timeSavingsTitle')}</h4>
                    ...
                    <Card className="bg-muted/30">
                      <CardContent className="pt-6">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <h4 className="font-medium">{t('landing.demo.interestSavingsTitle')}</h4>
                            <div className="mt-1 space-y-1">
                              <p className="text-2xl font-bold">
                        ...
                        <p className="text-sm">
                          {strategy === 'avalanche' 
                            ? t('landing.demo.avalancheComparisonNote', { 
                                months: monthsDiff, 
                                interest: currencyFormatter.format(interestDiff) 
                              })
                ...
                {/* Chart visualization */}
                <div className="border-t pt-6">
                  <h3 className="text-lg font-medium mb-4">{t('landing.demo.visualizationTitle')}</h3>
                ...
                <div className="border-t pt-6 text-center">
                  <h3 className="text-lg font-medium mb-2">{t('landing.demo.ctaTitle')}</h3>
