 ...
 */
export const calculateCreditCardSummary = (
	cards: CreditCard[]
): {
	totalBalance: number;
	totalLimit: number;
 ...
 */
export const formatPayoffTime = (
	months: number | null,
	t: (key: string) => string
): string => {
	if (months === null) {
		return t("loan.customPaymentTermEstimateNever");
	}

	if (months === 0) {
		return t("creditCard.table.paid");
	}

	if (months === 1) {
		return `1 ${t("form.months.month")}`;
	}

	if (months < 12) {
		return `${months} ${t("form.months.months")}`;
	}
	...
	return `${years} ${t("table.years")} ${remainingMonths} ${t(
		"form.months.months"
	)}`;
};
